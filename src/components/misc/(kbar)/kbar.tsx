...
const Content = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <div
      className={`w-[97vw] rounded-2xl bg-white p-6 shadow-lg outline-none dark:bg-neutral-1000 md:w-[45vw] ${results.length > 0 && 'space-y-4'
        }`}
    >
      {children}
    </div>
...
>(({ action, active }, ref) => {
  return (
    <div
      ref={ref}
      className={`m-0 flex cursor-pointer items-center justify-between gap-5 rounded-xl p-2 text-lg transition-colors ${active
